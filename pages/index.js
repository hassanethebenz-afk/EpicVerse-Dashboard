...
export default function Home() {
  ...
  return (
    <div style={{ display: "flex", height: "100vh", fontFamily: "'Orbitron', sans-serif", background: "linear-gradient(135deg, #0f0c29, #302b63, #24243e)", color: "#00ffea" }}>
      <div style={{ width: "220px", background: "rgba(0,0,0,0.55)", padding: "20px", display: "flex", flexDirection: "column", gap: "25px" }}>
        <h2>EpicVerse</h2>
      ...
      <div style={{ flex: 1, overflowY: "auto", padding: "30px" }}>
        <div style={{ background: "rgba(0,0,0,0.45)", borderRadius: "15px", padding: "20px", marginBottom: "20px", textAlign: "center", boxShadow: "0 0 30px rgba(0,255,234,0.5)" }}>
          <h3>{t[lang].botStatus}</h3>
          <p style={{ fontSize: "2rem", fontWeight: "bold", color: "#ff00ff" }}>{botStatus}</p>
        </div>
        <div style={{ background: "rgba(0,0,0,0.45)", borderRadius: "15px", padding: "20px", marginBottom: "20px", textAlign: "center", boxShadow: "0 0 30px rgba(0,255,234,0.5)" }}>
          <h3>{t[lang].serverCount}</h3>
          <p style={{ fontSize: "2rem", fontWeight: "bold", color: "#ff00ff" }}>{serverCount}</p>
        </div>
        <div style={{ background: "rgba(0,0,0,0.45)", borderRadius: "15px", padding: "20px", marginBottom: "20px", textAlign: "center", boxShadow: "0 0 30px rgba(0,255,234,0.5)" }}>
          <h3>{t[lang].userCount}</h3>
          <p style={{ fontSize: "2rem", fontWeight: "bold", color: "#ff00ff" }}>{userCount}</p>
        </div>
        <div style={{ background: "rgba(0,0,0,0.45)", borderRadius: "15px", padding: "20px", marginBottom: "20px", textAlign: "center", boxShadow: "0 0 30px rgba(0,255,234,0.5)" }}>
          <h3>{t[lang].commandsCount}</h3>
          <p style={{ fontSize: "2rem", fontWeight: "bold", color: "#ff00ff" }}>{commandsCount}</p>
        </div>
        <div style={{ background: "rgba(0,0,0,0.45)", borderRadius: "15px", padding: "20px", marginBottom: "20px", textAlign: "center", boxShadow: "0 0 30px rgba(0,255,234,0.5)" }}>
          <h3>{t[lang].liveCommands}</h3>
          <table style={{ width: "100%", borderCollapse: "collapse", marginTop: "20px", color: "#00ffea" }}>
            <thead>
              <tr><th>Command</th><th>Description</th></tr>
            </thead>
            <tbody>
              {commands.map((cmd, i) => <tr key={i}><td>{cmd.name}</td><td>{cmd.description}</td></tr>)}
            </tbody>
          </table>
        </div>
        <button style={{ marginTop: "10px", padding: "10px 20px", borderRadius: "12px", background: "linear-gradient(145deg, #00ffea, #ff00ff)", border: "none", cursor: "pointer", fontWeight: "bold", color: "#000" }} onClick={toggleLang}>
          {t[lang].translateBtn}
        </button>
      </div>
    </div>
